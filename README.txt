...
A plugin that allows Jenkins/Hudson to build merge requests.
...
* Whilst there is no explict dependency on the [Git plugin](https://wiki.jenkins-ci.org/display/JENKINS/Git+Plugin),
  it's strongly recommended that you install it since Jenkins/Hudson will be unable to fetch the source code for your project.

## Installation

* Ensure that a Jenkins/Hudson user exists within Gitlab and has access to the repository. Ensure that the user
  has **Developer** level access to the project.
* Install the plugin in Jenkins/Hudson.
    * The plugin is hosted on the [Jenkins/Hudson Plugin repository](https://wiki.jenkins-ci.org/display/JENKINS/Gitlab+Merge+Request+Builder+Plugin)
    * Go to ``Jenkins/Hudson`` -> ``Manage Plugins`` -> ``Available``
    * Search for ``Gitlab Merge Request Builder``
    * And install it
    * Ensure you restart Jenkins/Hudson
* Go to ``Manage Jenkins/Hudson`` -> ``Configure System`` -> ``Gitlab Merge Requests Builder``
* Set the ``Gitlab Host URL`` to the base URL of your Gitlab server
* Set your ``Jenkins/Hudson Username`` for the Jenkins/Hudson user (defaults to jenkins)
* Set your ``Jenkins/Hudson API Token`` for the Jenkins/Hudson user. This can be found by logging into Gitlab as Jenkins/Hudson User
